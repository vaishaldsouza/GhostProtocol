...
export const ResendDispatchModal: React.FC<ResendDispatchModalProps> = ({
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSending(false);
      if (res.success) {
        setSendSuccessMessage(`✅ Transactional Email dispatched to ${donorEmail}! Provider: ${res.log.provider}`);
        refreshLogs();
        setTimeout(() => {
          setActiveTab('logs');
        }, 1200);
      ...
      setIsSending(false);
      setSendSuccessMessage(`❌ Error: ${error instanceof Error ? error.message : 'Failed to send email'}`);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-slate-900 w-full max-w-5xl rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        {/* Modal Header */}
        <div className="px-6 py-4 bg-gradient-to-r from-blue-600 via-cyan-700 to-slate-900 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-white/10 rounded-2xl backdrop-blur-md border border-white/20">
              <Mail className="w-6 h-6 text-blue-200 fill-blue-200/20" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-full bg-blue-400 text-slate-950 text-[10px] font-black uppercase">
                  Gmail SMTP (Free)
                </span>
              ...
              <h3 className="text-lg font-black tracking-tight mt-0.5">
                RedPulse Gmail SMTP Email Dispatcher &amp; Delivery Tracker
              </h3>
            </div>
          </div>
        ...
        <div className="flex items-center border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 px-6 pt-3 gap-2 overflow-x-auto">
          ...
          >
            ...
            <span className="px-2 py-0.2 bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 text-[10px] rounded-full font-black">
              {logs.length}
            </span>
          </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <form onSubmit={handleSendEmail} className="lg:col-span-7 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-blue-600 dark:text-blue-400 font-bold">
                    Gmail SMTP Transactional Template
                  </span>
                </div>
                ...
                {!hasEmailCreds && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-950/60 border border-blue-200 dark:border-blue-800 rounded-2xl text-xs">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="font-bold text-blue-900 dark:text-blue-100 mb-1">Free Gmail SMTP Mode Active</p>
                        <p className="text-blue-700 dark:text-blue-300">
                          Using Gmail SMTP via Nodemailer — completely free! Your Gmail credentials are configured in the Config tab or .env file.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-3 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-900/60 rounded-2xl text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center justify-between">
                  <span>Subject line preset:</span>
                  <span className="px-2.5 py-1 bg-blue-600 text-white rounded-xl text-[11px] font-black">
                ...
                <div className="pt-2">
                  <button
                    type="submit"
                    disabled={isSending}
                    className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 active:scale-[0.99] text-white font-extrabold text-xs rounded-2xl shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 transition disabled:opacity-50"
                  >
                    {isSending ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Send Transactional Email via Gmail SMTP</span>
                      </>
                    )}
                  </button>
                </div>
              ...
              <div className="lg:col-span-5 bg-slate-900 p-5 rounded-3xl border border-slate-800 text-white flex flex-col justify-between">
                ...
                <div className="mt-4 pt-3 border-t border-slate-800 text-[10px] text-slate-400 text-center">
                  Powered by Gmail SMTP (Nodemailer) • Free Transactional Email
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
                <div className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  ...
                  <span>Rendered Transactional Email Preview (Gmail SMTP HTML Template)</span>
                </div>
                <button
              ...
              <div className="w-full rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-slate-950 shadow-inner min-h-[480px]">
                <iframe
                  title="Gmail SMTP Transactional Email Preview"
                  ...
                  {filteredLogs.map((log) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100 dark:border-slate-800">
                        <span>Gmail SMTP Message ID: {log.resendEmailId || 'smtp_sandbox_id'}</span>
                        <a
                          href={log.acceptLink}
                          target="_blank"
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium hidden sm:block">
            RedPulse Transactional Email Infrastructure • Powered by Gmail SMTP (Free)
          </div>
